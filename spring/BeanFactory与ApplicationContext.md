关系：接口ApplicationContext是接口BeanFactory的子类

接口BeanFactory功能：

```java
//前缀  区分用BeanFactory返回的实例 和 其他工厂返回实例
String FACTORY_BEAN_PREFIX = "&";
Object getBean(String name) throws BeansException;
/** * @param requiredType type the bean must match; can be an interface or superclass */<T> T getBean(String name, Class<T> requiredType) throws BeansException;
...
/**获取别名，bean声明的时候可确定别名，这里可以获取 */
String[] getAliases(String name);
```

接口ApplicationContext功能：
...
接口ApplicationContext子类：

	* ClassPathXmlApplicationContext
	* FileSystemXmlApplicationContext
	* XmlWebApplicationContext

接口AutowireCapableBeanFactory功能：主要功能是装配applicationContext之外的Bean
